...
export default function Dashboard() {
  ...
  const handleSubmit = (e) => {
    ...
    setProfile({ ...formData });
    setEditMode(false);
    alert('Profile updated!'); // Replace with API call later
  };

  const handleDelete = () => {
    if (window.confirm('Are you sure you want to delete your profile?')) {
      setProfile(null);
      alert('Profile deleted!'); // Replace with API call later
    }
  };
        ...
        <div>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
            ...
            <div className="flex gap-4">
              <Button type="submit">{editMode ? 'Update Profile' : 'Create Profile'}</Button>
              {editMode && (
                <Button
                  type="button"
